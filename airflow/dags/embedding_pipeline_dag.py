...
def embedding_pipeline_dag():
    ...
    # ------------------------------------------------------------------
    @task()
    def embed_and_upsert(filings: list[dict]) -> list[dict]:
        """Run the embedding pipeline for each pending filing.
        ...
        log.info("Embedding complete: %d / %d filings", len(results), len(filings))
        return results
    ...
    # ------------------------------------------------------------------
    @task()
    def log_index_stats(results: list[dict]) -> None:
        ...
        log.info("Embedding results summary: %s", results)

    # ------------------------------------------------------------------
    # Wire the DAG
    # ------------------------------------------------------------------
    pending = get_pending_filings()
    embedded = embed_and_upsert(pending)
